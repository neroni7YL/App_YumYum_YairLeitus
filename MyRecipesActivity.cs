...
{
    [Activity(Label = "MyRecipesActivity")]
    public class MyRecipesActivity : Activity, AdapterView.IOnItemClickListener
    {
        // "רשימת כל המתכונים שהמשתמש יצר - "המתכונים שלי
        private List<Recipe> recipeList;

        // רשימת המתכונים שלי המתעדכנת לפי סינון או חיפוש
        ...
        protected override void OnCreate(Bundle savedInstanceState)
        {
            ...
            SetContentView(Resource.Layout.myRecipes_layout);

            lvRecipes = FindViewById<ListView>(Resource.Id.myRecipes_lvRecipes);
            lvRecipes.OnItemClickListener = this; // Click on view - recipe

            actvSearch = FindViewById<AutoCompleteTextView>(Resource.Id.myRecipes_actvSearch);
            btnClear = FindViewById<Button>(Resource.Id.myRecipes_btnClear);
            btnSearch = FindViewById<Button>(Resource.Id.myRecipes_btnSearch);
            btnSort = FindViewById<Button>(Resource.Id.myRecipes_btnSort);
            btnGoBack = FindViewById<Button>(Resource.Id.myRecipes_btnGoBack);
        ...
        private void UpdateRecipeNameAdapter()
        {
            ...
            // recipeNameList מעבר על כל המתכונים שלי והוספת שמות המתכונים לרשימת   
        ...
        /* OnCreaete() פעולה שמתרחשת לאחר  
         כך שאם המשתמש עדכן או מחק מתכון שיצר OnRestart() הפעולה נקראת לאחר 
        ...
        /* לאחר בחירת קטגוריות לסינון CategoryDialogFragment פעולה שנקראת ב  
         הפעולה מקבלת את כל המתכונים שלי ומסננת כך שרשימת המתכונים
        ...
        public void ReceiveRecipeTypeDetails(string recipeType, string[] details)
        {
            ...
            {
                // קבלת כל הסלטים שלי 
                recipeList = DatabaseManager.GetAllMySalads();
            ...
            if (recipeType == "Soup")
            {
                ...
                recipeList = DatabaseManager.GetAllMySoups();
            ...
            if (recipeType == "Meat")
            {
                ...
                recipeList = DatabaseManager.GetAllMyMeats();
            ...
            if (recipeType == "Pastry")
            {
                ...
                recipeList = DatabaseManager.GetAllMyPastries();
        ...
        // המכיל את אפשרויות הסינון מתכון SortByDialogFragment פתיחת 
